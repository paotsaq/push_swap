...
static void	gets_distances(best_moves *list, s_stacks *stacks, int e_pos)
{
	...
	l_pos = pos_in_array(stacks->lis, stacks->b[e_pos], stacks->lis_size);
	...
	l_pos = pos_in_array(stacks->a, stacks->lis[l_pos], stacks->lis_size);
...
void	best_move_interface(s_stacks *stacks)
{
	...
	e_pos = -1;
	while (++e_pos < stacks->b_count && list.index++)
